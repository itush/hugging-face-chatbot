    ...
    <div className="max-w-lg mx-auto p-4 border rounded-lg shadow-lg">
      <div className="h-80 overflow-y-auto mb-4 border-b p-2">
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`p-2 rounded-md my-1 ${msg.role === "user" ? "bg-blue-500 text-white" : "bg-gray-900"}`}
          >
            {msg.content}
          </div>
        ))}
      ...
      <div className="flex">
        <input
          ...
          disabled={loading}
        />
        <button onClick={sendMessage} className="p-2 bg-blue-500 text-white rounded-r-md" disabled={loading}>
